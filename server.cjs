...
async function checkAndInvalidateStaleCspChunks() {
    ...
    try {
        ...
        try {
            ...
            console.warn('🔍 [CSP-Stale-Check] Could not get chain height, using default:', chainHeight);
        }
...
async function fillMissingCspChunks(missingChunks, chainHeight = null) {
    if (cspFillInProgress) {
        console.log('🔄 [CSP-Fill] Already in progress, skipping');
        return;
    }
...
app.post(['/api/wallet/get_outs.bin', '/vault/api/wallet/get_outs.bin'], express.raw({ limit: '10mb', type: '*/*' }), async (req, res) => {
    ...
    const daemonNodes = process.env.SALVIUM_RPC_URL
        ? [process.env.SALVIUM_RPC_URL]
        : ['http://salvium:19081', 'http://seed01.salvium.io:19081'];
    ...
    try {
        ...
        if (jsonResponse.data && jsonResponse.data.status === 'OK' && jsonResponse.data.outs) {
            ...
            console.log(`🔍 [Wallet API] Comparing request_index vs daemon_output_id:`);
            for (let i = 0; i < Math.min(5, responseOuts.length, outputs.length); i++) {
                const reqIdx = outputs[i].index;
                const daemonOutId = responseOuts[i].output_id;
                const match = reqIdx === daemonOutId ? '✓' : '✗ DIFFERENT';
                console.log(`   [${i}] request_index=${reqIdx}, daemon_output_id=${daemonOutId} ${match}`);
            }
            ...
            console.log(`✅ [Wallet API] /get_outs succeeded, got ${responseOuts.length} outputs for asset_type='${assetType}'`);
            res.set('Content-Type', 'application/json');
            return res.json(jsonResponse.data);
        } else {
            console.error(`❌ [Wallet API] /get_outs returned invalid response:`, jsonResponse.data?.status);
            res.status(500).json({ error: 'Invalid response from daemon' });
        }
    } catch (err) {
        console.error(`❌ [Wallet API] /get_outs failed: ${err.message}`);
        res.status(500).json({ error: err.message });
    }
});
...
app.post(['/api/wallet-rpc/getblocks.bin', '/api/wallet-rpc/gethashes.bin', '/getblocks.bin', '/gethashes.bin'], express.raw({ limit: '50mb', type: '*/*' }), async (req, res) => {
    ...
    const requestId = req.headers['x-request-id'] || `server-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
...
app.post(['/api/wallet/get_random_outs', '/vault/api/wallet/get_random_outs'], express.json(), async (req, res) => {
    ...
    try {
        ...
        while (uniqueIndices.size < count + 50) {
            const gamma = -Math.log(Math.random()) * 1296;
        ...
        const outsResponse = await axiosInstance.post(DAEMON_URL.replace(/\/$/, '') + '/get_outs', {
            outputs: randomIndices.slice(0, count + 50),
            get_txid: false
        }, { timeout: 60000 });

        const validOuts = (outsResponse.data?.outs || []).filter(out => out && out.key);
        console.log('[Wallet API] get_random_outs succeeded: ' + validOuts.length + ' outputs');

        res.json({ outs: validOuts.slice(0, count), status: 'OK' });
    } catch (error) {
        console.error('[Wallet API] get_random_outs failed:', error.message);
        console.error('[Wallet API] get_random_outs error details:', error.response?.data || error.stack);
...
app.post(['/api/wallet/sendrawtransaction', '/vault/api/wallet/sendrawtransaction'], async (req, res) => {
    res.header('Access-Control-Allow-Origin', '*');
    res.header('Access-Control-Allow-Methods', 'POST, OPTIONS');
    res.header('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept');
