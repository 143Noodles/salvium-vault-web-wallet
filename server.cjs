...
// Can be overridden by explicit per-feature env vars.
...
async function daemonHttpPost(path, payload) {
    const baseUrl = (process.env.SALVIUM_RPC_URL || RPC_NODES[0] || 'http://salvium:19081').replace(/\/$/, '');
    const config = {
        method: 'POST',
        url: `${baseUrl}${path}`,
        headers: {
...
app.get(['/api/wallet/stake-return-heights', '/vault/api/wallet/stake-return-heights'], async (req, res) => {
    ...
    try {
        // Optional safety mode (test deployments): disable stake-return filtering
        // so scanner never suppresses legitimate miner rewards in local chains.
