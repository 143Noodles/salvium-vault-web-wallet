...
const Onboarding: React.FC<OnboardingProps> = ({ onComplete }) => {
  ...
  useEffect(() => {
    const fetchHeight = async () => {
      try {
        ...
        console.error('Failed to fetch daemon height:', e);
      }
    };
  ...
  const handleRestoreWallet = async () => {
    ...
    try {
      ...
      await wallet.restoreWallet(restoreSeed.trim(), password, height);
      onComplete('restore');
    } catch (err: any) {
  ...
  const handleDecryptBackup = async () => {
    ...
    try {
      ...
      try {
        ...
        console.error('Failed to auto-unlock after restore:', unlockErr);
  ...
  if (mode === 'initial') {
    return (
      <div className="flex items-center justify-center p-4 bg-[#0b0b15] relative h-[100dvh] overflow-y-auto" style={{}}>
  ...
  if (mode === 'create') {
    return (
      <div className="flex items-center justify-center p-4 bg-[#0b0b15] h-[100dvh] overflow-hidden">
  ...
  if (mode === 'restore') {
    return (
      <div className="flex items-center justify-center p-4 bg-[#0b0b15] h-[100dvh] overflow-y-auto">
