...
export const BiometricService = {
    ...
    isAvailable: async (): Promise<boolean> => {
        ...
        try {
            ...
            console.warn('[BiometricService] Availability check failed:', e);
            return false;
        }
    },
    ...
    enable: async (password: string): Promise<boolean> => {
        try {
            ...
            console.error('[BiometricService] Enable failed:', e);
    ...
    authenticate: async (): Promise<string | null> => {
        try {
            ...
            console.error('[BiometricService] Auth failed:', e);
            return null;
        }
    }
};
...
async function getPRFKey(credentialIdBase64: string): Promise<Uint8Array | null> {
    try {
        ...
        if (!prfResults?.first) {
            console.warn('[BiometricService] PRF results not available');
            return null;
        }
        ...
        console.error('[BiometricService] PRF key derivation failed:', e);
        return null;
    }
}
...
async function verifyBiometric(credentialIdBase64: string): Promise<boolean> {
    try {
        ...
        console.error('[BiometricService] Biometric verification failed:', e);
        return false;
    }
}
