...
export class WalletService {
  ...
  private async loadWasm(): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      ...
      if (typeof SharedArrayBuffer === 'undefined') {
        console.warn('[WalletService] SharedArrayBuffer not available - using single-threaded fallback for iOS/Safari');
        // @ts-ignore - Polyfill SharedArrayBuffer with regular ArrayBuffer
      ...
      script.onerror = (event) => {
        console.error('[WalletService] Failed to load WASM script from:', wasmUrl);
        console.error('[WalletService] Error event:', event);
  ...
  setBlockchainHeight(height: number, advanceWallet: boolean = false): void {
    ...
    try {
      ...
      console.error('[WalletService] Failed to set blockchain height:', e);
    }
  }
  ...
  getBalance(): BalanceInfo {
    ...
    try {
      ...
      const unlockedBalance = parseInt(unlockedStr) || 0;

      return {
        balance,
        unlockedBalance,
        balanceSAL: balance / ATOMIC_UNITS,
        unlockedBalanceSAL: unlockedBalance / ATOMIC_UNITS,
      };
    } catch (e) {
      console.error('[WalletService] Failed to get balance:', e);
      return { balance: 0, unlockedBalance: 0, balanceSAL: 0, unlockedBalanceSAL: 0 };
    }
  }
  ...
  getSyncStatus(): SyncStatus {
    ...
    try {
      ...
      console.error('[WalletService] Failed to get sync status:', e);
      return { walletHeight: 0, daemonHeight: 0, isSyncing: false, progress: 0 };
    }
  }
  ...
  getTransactions(): WalletTransaction[] {
    ...
    try {
      ...
      console.error('[WalletService] Failed to get transactions:', e);
      return [];
    }
  }
  ...
  async estimateFee(address: string, amount: number, priority: number = 1): Promise<number> {
    try {
      ...
      console.warn('[WalletService] Failed to fetch dynamic fee:', e);
    }
  ...
  private async _createAndBroadcastTransaction(
    ...
    try {
      ...
      if (this.wasmModule?.inject_decoy_outputs_from_json) {
        ...
        console.warn('[WalletService] inject_decoy_outputs_from_json not available in WASM!');
      }
      ...
      if (broadcastResult.status !== 'OK') {
        console.error('[WalletService] Broadcast REJECTED:', broadcastResult);
        throw new Error(broadcastResult.reason || broadcastResult.error || 'Broadcast rejected by network');
      }

      return txHash;

    } catch (e) {
      console.error('[WalletService] Failed to send transaction:', e);
      throw e;
    }
  }
    ...
    try {
      ...
      while (fetchRound < MAX_FETCH_ROUNDS) {
        ...
        try {
          ...
          console.warn(`[WalletService] Stake attempt ${fetchRound} failed: ${lastError}`);
        }
      }
      ...
      if (broadcastResult.status !== 'OK') {
        console.error('[WalletService] Stake broadcast REJECTED:', broadcastResult);
  ...
  private async fetchAndInjectExactOutputs(requestBase64: string): Promise<void> {
    ...
    const response = await fetch('/vault/api/wallet/get_outs.bin', {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error('[WalletService] get_outs.bin proxy failed:', response.status, errorText);
  ...
  private async prepareDecoys(): Promise<void> {
    ...
    for (let i = 0; i < numOutputsNeeded; i++) {
    ...
    if (this.wasmModule?.inject_decoy_outputs_from_json) {
      ...
      if (jsonData.outs && Array.isArray(jsonData.outs) && jsonData.outs.length === outputIndices.length) {
        ...
        console.warn(`[WalletService] Mismatch between requested indices (${outputIndices.length}) and returned outputs (${jsonData.outs?.length}) - cache keying may fail`);
      }
      ...
      console.warn('[WalletService] inject_decoy_outputs_from_json not available, falling back to base64');
      // Fallback to base64 approach (unlikely to work if we got here, but try anyway)
  ...
  private async fetchRpc(method: string, params: any = {}): Promise<any> {
    try {
      ...
      console.warn(`[WalletService] RPC ${method} failed:`, e);
      return null;
    }
  }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (resultData.distributions?.length > 0) {
          ...
          console.warn('[WalletService] Distribution response has no data:', resultData.error || 'empty');
        }
      } else {
        const errorText = await response.text().catch(() => 'Unknown error');
        console.warn('[WalletService] Failed to fetch output distribution:', response.status, errorText);
      }
    } catch (e) {
      console.warn('[WalletService] Failed to inject output distribution:', e);
    }
  }
  ...
  private async getDaemonHeight(): Promise<number> {
    try {
      ...
      console.warn('[WalletService] Could not get daemon height:', e);
    }
    return 0; // Return 0 on failure so we don't assume a high height
  }
  ...
  createSubaddress(label: string = ''): string {
    ...
    try {
      return this.walletInstance.create_subaddress(0, label);  // Account 0 (default)
    } catch (e) {
      console.error('[WalletService] Failed to create subaddress:', e);
      throw e;
    }
  }
  ...
  getSubaddresses(): Array<{ address: string; label: string; index: { major: number; minor: number }; balance: number; unlocked_balance: number }> {
    ...
    try {
      ...
      console.error('[WalletService] Failed to get subaddresses:', e);
      return [];
    }
  }
  ...
  exportOutputs(): { outputs_hex: string; count: number } | null {
    if (!this.walletInstance || !this.walletInstance.is_initialized()) {
      console.warn('[WalletService] Cannot export outputs: wallet not initialized');
      return null;
    }
    ...
    try {
      ...
      if (result.status === 'success') {
        ...
        console.error('[WalletService] Export outputs failed:', result.error);
        return null;
      }
    } catch (e) {
      console.error('[WalletService] Failed to export outputs:', e);
      return null;
    }
  }
  ...
  importOutputs(outputs_hex: string): number {
    if (!this.walletInstance || !this.walletInstance.is_initialized()) {
      console.warn('[WalletService] Cannot import outputs: wallet not initialized');
      return -1;
    }

    if (!outputs_hex || outputs_hex.length === 0) {
      console.warn('[WalletService] Cannot import outputs: empty hex string');
      return 0;
    }
    ...
    try {
      ...
      if (result.status === 'success') {
        ...
        console.error('[WalletService] Import outputs failed:', result.error);
        return -1;
      }
    } catch (e) {
      console.error('[WalletService] Failed to import outputs:', e);
      return -1;
    }
  }
  ...
  exportWalletCache(): { cache_hex: string } | null {
    if (!this.walletInstance || !this.walletInstance.is_initialized()) {
      console.warn('[WalletService] Cannot export wallet cache: wallet not initialized');
      return null;
    }
    ...
    try {
      ...
      if (typeof this.walletInstance.export_wallet_cache_hex !== 'function') {
        console.warn('[WalletService] export_wallet_cache_hex not available, falling back to export_outputs_hex');
      ...
      if (result.status === 'success') {
        ...
        console.error('[WalletService] Export wallet cache failed:', result.error);
        return null;
      }
    } catch (e) {
      console.error('[WalletService] Failed to export wallet cache:', e);
      return null;
    }
  }
  ...
  importWalletCache(cache_hex: string): boolean {
    if (!this.walletInstance || !this.walletInstance.is_initialized()) {
      console.warn('[WalletService] Cannot import wallet cache: wallet not initialized');
      return false;
    }

    if (!cache_hex || cache_hex.length === 0) {
      console.warn('[WalletService] Cannot import wallet cache: empty hex string');
      return false;
    }
    ...
    try {
      ...
      if (typeof this.walletInstance.import_wallet_cache_hex !== 'function') {
        console.warn('[WalletService] import_wallet_cache_hex not available, falling back to import_outputs_hex');
      ...
      if (result.status === 'success') {
        ...
        console.error('[WalletService] Import wallet cache failed:', result.error);
        return false;
      }
    } catch (e) {
      console.error('[WalletService] Failed to import wallet cache:', e);
      return false;
    }
  }
  ...
  getKeyImages(): string[] {
    ...
    try {
      if (typeof this.walletInstance.get_key_images_csv === 'function') {
        ...
        if (
          ...
          for (let i = 0; i < chunkCount; i++) {
            csv += (this.walletInstance as any).get_key_images_csv_chunk(i, chunkSize);
          }
        } else {
      ...
      console.error('[WalletService] Failed to get key images:', e);
      return [];
    }
  }
  ...
  getSpentKeyImages(): Record<string, number> {
    ...
    try {
      ...
      if (typeof walletAny.get_key_images !== 'function') {
        console.warn('[WalletService] get_key_images not available');
        return {};
      }
      ...
      if (data.error) {
        console.error('[WalletService] get_key_images error:', data.error);
        return {};
      }
      ...
      console.error('[WalletService] Failed to get spent key images:', e);
      return {};
    }
  ...
  markOutputsSpent(spentKeyImages: Record<string, number>): number {
    if (!this.walletInstance || !this.walletInstance.is_initialized()) {
      console.warn('[WalletService] Cannot mark outputs spent: wallet not initialized');
      return 0;
    }
    ...
    try {
      ...
      if (typeof this.walletInstance.mark_spent_by_key_images === 'function') {
        ...
        if (result.error) {
          console.error('[WalletService] mark_spent_by_key_images error:', result.error);
          return 0;
        }

        return result.marked || 0;
      } else {
        console.warn('[WalletService] mark_spent_by_key_images not available in WASM');
        return 0;
      }
    } catch (e) {
      console.error('[WalletService] Failed to mark outputs spent:', e);
      return 0;
    }
  }
  ...
  async validateMnemonic(mnemonic: string): Promise<SeedValidationResult> {
    ...
    return new Promise(async (resolve) => {
      ...
      const timeout = setTimeout(() => {
        worker.terminate();
        console.error('[WalletService] Seed validation timed out after 30s');
  ...
  getDiagnostics(): any {
    ...
    try {
      ...
      console.error('[WalletService] Failed to get diagnostics:', e);
      return null;
    }
  }
  ...
  private connectBlockStream(): void {
    ...
    try {
          ...
          if (data.type === 'new_block') {
            ...
            for (const callback of this.newBlockCallbacks) {
              try {
                ...
                console.error('[WalletService] Callback error:', err);
              }
            }
          }
        } catch (err) {
          console.warn('[WalletService] Failed to parse block stream event:', err);
        }
      };

      this.blockStreamConnection.onerror = (error) => {
        console.warn('[WalletService] Block stream error:', error);
      ...
      console.error('[WalletService] Failed to create block stream connection:', err);
    }
  }
  ...
  scanTransaction(txBlobHex: string): boolean {
    if (!this.walletInstance || !this.walletInstance.is_initialized()) {
      console.error('[WalletService] cannot scan, wallet not initialized');
      return false;
    }
  ...
  getMempoolTxInfo(txBlobHex: string): any {
    ...
    try {
      ...
      console.error("[WalletService] Error in getMempoolTxInfo:", err);
      return {};
    }
  }
  ...
  private connectMempoolStream(): void {
    ...
    try {
          ...
          if (data.type === 'mempool_add' || data.type === 'mempool_remove') {
            ...
            for (const callback of this.mempoolTxCallbacks) {
              try {
                ...
                console.error('[WalletService] Mempool callback error:', err);
              }
            }
          }
        } catch (err) {
          console.warn('[WalletService] Failed to parse mempool event:', err);
        }
      };

      this.mempoolStreamConnection.onerror = (error) => {
      ...
      console.error('[WalletService] Failed to create mempool stream connection:', err);
      this.mempoolReconnecting = false;
    }
  }
  ...
  debugInputCandidates(): object | null {
    if (!this.walletInstance) {
      console.error('[WalletService] Wallet not initialized');
      return null;
    }
    try {
      ...
      console.error('[WalletService] debug_input_candidates failed:', e);
      return null;
    }
  }
  ...
  debugTxInputSelection(fromAccount: number = 0): object | null {
    ...
    try {
      ...
      console.error('[WalletService] debug_tx_input_selection failed:', e);
      return null;
    }
  }
  ...
  debugCreateTxPath(destAddress: string, amountStr: string): object | null {
    ...
    try {
      ...
      console.error('[WalletService] debug_create_tx_path failed:', e);
      return null;
    }
  }
  ...
  debugFeeParams(): object | null {
    ...
    try {
      ...
      console.error('[WalletService] debug_fee_params failed:', e);
      return null;
    }
  }
  ...
  async deleteWalletFile(): Promise<void> {
    ...
    if (!module) {
      console.warn('[WalletService] ❌ deleteWalletFile aborted: wasmModule is null/undefined');
      return;
    }
    if (!module.FS) {
      console.warn('[WalletService] ❌ deleteWalletFile aborted: FS module not found in wasmModule');
      return;
    }
    ...
    try {
      ...
      try {
        ...
        if (!lookup.exists) {
          console.warn(`[WalletService] Mount point ${MOUNT_POINT} does not exist. (Path: ${lookup.path}). Scanning root...`);
          targetDir = '/'; // Fallback to root if wallets dir is gone
        }
      } catch (e) {
        console.warn(`[WalletService] Failed to analyze path ${MOUNT_POINT}:`, e);
        targetDir = '/';
      }
      ...
      try {
        ...
        await new Promise<void>((resolve) => {
          FS.syncfs(false, (err: any) => {
            if (err) console.error('[WalletService] Failed to sync FS deletion:', err);
            resolve();
          });
        });

      } catch (e) {
        console.warn(`[WalletService] Error traversing ${targetDir}:`, e);
      }
    } catch (e) {
      console.error('[WalletService] Critical error in deleteWalletFile:', e);
    }
  }
}
