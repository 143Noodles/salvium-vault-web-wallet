...
async function handleLoadWasm(msg) {
    try {
        ...
        if (DEBUG) {
            console.log(`[CSP Worker] WASM loaded: ${version}`);
            console.log(`[CSP Worker] Functions: scan_csp_batch=${hasScanCspBatch}, allocate=${hasAllocate}, compute_view_tag=${hasComputeViewTag}`);
        }

        if (!hasScanCspBatch) {
            console.error('[CSP Worker] CRITICAL: scan_csp_batch function not found in WASM module!');
            console.error('[CSP Worker] Available functions:', Object.keys(Module).filter(k => typeof Module[k] === 'function' && !k.startsWith('_')).slice(0, 20).join(', '));
        }
...
async function handleInit(msg) {
    ...
    if (DEBUG) console.log(`[CSP Worker ${workerId}] Init: viewKey=${viewSecretKey.substring(0, 8)}..., kViewIncoming=${hasCarrotKey ? kViewIncoming.substring(0, 8) + '...' : 'NONE'}, sViewBalance=${(sViewBalance && sViewBalance.length === 64) ? (sViewBalance.substring(0, 8) + '...') : 'NONE'}, keyImages=${hasKeyImages ? 'YES' : 'NO'}, stakeHeights=${hasStakeFilter ? msg.stakeReturnHeights.length + ' heights' : 'NONE'}, subaddresses=${hasOwnershipCheck ? subaddressCount : 'NONE'}`);
    if (!hasCarrotKey) {
        console.warn(`[CSP Worker ${workerId}] ⚠️ NO Carrot key provided - Carrot transactions will NOT be filtered properly!`);
    }
    if (hasOwnershipCheck && DEBUG) {
        console.log(`[CSP Worker ${workerId}] ✅ Ownership verification ENABLED - Phase 1 will return only verified matches`);
    }
    ...
    try {
        ...
        const mergeResult = (jsonStr) => {
            ...
            try {
                ...
                console.error('Scan result parse failed:', e);
            }
        };
        ...
        if (scanMs > 1000) {
            console.warn(`[CSP Worker ${workerId}] SLOW scan_csp_batch: ${scanMs.toFixed(0)}ms for chunk ${startHeight} (${cspBuffer.byteLength} bytes, ${txCount} txs)`);
        }
...
async function handleScanCspBatch(msg) {
    ...
    if (DEBUG) console.log(`[CSP Worker ${workerId}] 🔄 Starting batch ${startHeight} (${chunkCount} chunks)`);
    ...
    try {
        ...
        if (isNearTip) {
            if (DEBUG) console.log(`[CSP Worker ${workerId}] 🔥 Live edge batch - cache buster added`);
        }
        ...
        if (DEBUG) console.log(`[CSP Worker ${workerId}] 📥 Batch ${startHeight}: HTTP ${response.status} in ${tempFetchMs.toFixed(0)}ms`);
        ...
        if (response.status === 404) {
            console.warn(`[CSP Worker ${workerId}] Batch at ${startHeight} returned 404 (beyond chain tip) - ending scan gracefully`);
            self.postMessage({
        ...
        if (!response.ok) {
            ...
            if (startHeight >= 370000) { // Only for recent chunks
                console.warn(`[CSP Worker ${workerId}] Batch at ${startHeight} failed with ${response.status} - treating as end of chain`);
                self.postMessage({
                    type: 'SCAN_BATCH_RESULT',
                    workerId,
                    startHeight,
        ...
        while (offset < batchBuffer.byteLength) {
            ...
            if (offset + cspLength > batchBuffer.byteLength) {
                console.error(`[CSP Worker ${workerId}] Batch buffer overflow at chunk ${chunksProcessed}`);
                break;
            }
            ...
            if (!ptr) {
                console.error(`[CSP Worker ${workerId}] ❌ Failed to allocate for chunk ${chunkStartHeight}`);
                chunksProcessed++;
                continue;
            }
        ...
        console.error(`[CSP Worker ${workerId}] ❌ Batch ${startHeight} ${errorType}:`, err.message);
...
async function handleScanKeyImagesOnly(msg) {
    ...
    if (DEBUG) console.log(`[CSP Worker ${workerId}] 🔍 Phase 1b FAST: Scanning ${startHeight} for key images`);
    ...
    try {
        ...
        while (offset < batchBuffer.byteLength) {
            ...
            try {
                const result = JSON.parse(resultJson);
                if (result.error) {
                    console.warn(`[CSP Worker ${workerId}] Key image scan error in chunk: ${result.error}`);
                } else {
                ...
                console.warn(`[CSP Worker ${workerId}] Failed to parse key image result:`, e);
            }
        ...
        const errorType = err.name === 'AbortError' ? 'TIMEOUT' : 'ERROR';
        console.error(`[CSP Worker ${workerId}] ❌ Key image scan ${startHeight} ${errorType}:`, err.message);
