...
async function handleLoadWasm(msg) {
    try {
        ...
        console.log(`[Phase2 Worker] WASM loaded: ${version}`);
...
async function handleInit(msg) {
    try {
        ...
        if (subaddressSpendKeys && subaddressSpendKeys.length > 0 && wallet.init_view_only_with_map) {
            ...
            console.log(`[Worker ${workerId}] Initializing with precomputed CSV map (~${keyCount} keys), svb=${viewBalanceSecret ? 'YES' : 'NO'}, carrot_spend=${carrotSpendPubkey ? 'YES' : 'NO'}...`);
            ...
            console.log(`[Worker ${workerId}] Initializing with derivation lookahead ${count}...`);
        ...
        if (returnAddresses && returnAddresses.length > 0 && typeof wallet.add_return_addresses === 'function') {
            ...
            if (returnAddressCount >= 0) {
                console.log(`[Worker ${workerId}] Added ${returnAddressCount} return addresses from stake cache`);
            } else {
                console.warn(`[Worker ${workerId}] add_return_addresses returned error: ${returnAddressCount}`);
            }
        }
        ...
        console.log(`[Phase2 Worker ${workerId}] Init complete`);
...
async function handleProcessBatch(msg) {
    ...
    try {
        ...
        console.log(`[Worker ${workerId}] Starting WASM ingest_sparse_transactions...`);
        ...
        console.log(`[Worker ${workerId}] WASM complete: ${result.txs_matched || 0} matches, ${(result.matched_indices || []).length} indices in ${(ingestMs / 1000).toFixed(1)}s`);
