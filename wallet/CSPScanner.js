...
class CSPScanner {
    ...
    * @param {string} options.sViewBalance - 64-char hex Carrot s_view_balance secret (needed for internal enote tags)
    ...
    constructor(options) {
        ...
        // Warn if Carrot key is missing (Salvium uses Carrot by default!)
        if (!this.kViewIncoming || this.kViewIncoming.length !== 64) {
            console.warn('[CSPScanner] ⚠️ No Carrot k_view_incoming key provided! Carrot transactions will NOT be detected!');
        }
        ...
        this.onError = options.onError || ((e) => console.error('[CSPScanner]', e));
    ...
    async maybeAutoTune() {
        ...
        if (this.DEBUG) {
            console.log(`[CSPScanner] 🔧 AutoTune: workers=${this.enabledWorkerCount}/${this.maxWorkerCount}, medianChunkMs=${medianMs.toFixed(0)}, uiLag=${uiLag.toFixed(0)}ms`);
        }
    }
    ...
    async fetchWasmBinary() {
        ...
        if (this.DEBUG) console.log(`[CSPScanner] WASM binary: ${(this.wasmBinary.byteLength / 1024 / 1024).toFixed(2)} MB`);
    ...
    async streamCspBundle() {
        try {
            ...
            if (!response.ok) {
                console.log(`[CSPScanner] 📦 Bundle not available (${response.status}) - using chunk mode`);
                return null;
            }
            ...
            while (true) {
                ...
                if (!headerParsed && bufferLen >= 20) {
                    ...
                    if (magic !== 0x43535042) {
                        console.warn('[CSPScanner] 📦 Invalid bundle magic - using chunk mode');
                        return null;
                    }
            ...
            console.log('[CSPScanner] 📦 Stream failed:', err.message, '- using chunk mode');
            return null;
        }
    }
    ...
    async fetchCspBundle() {
        try {
            ...
            console.log('[CSPScanner] 📦 Bundle fetch failed:', err.message, '- using chunk mode');
            return null;
        }
    }
    ...
    extractChunkFromBundle(bundle, startHeight) {
        ...
        if (dataEnd > bundle.data.length) {
            console.warn(`[CSPScanner] Bundle chunk ${startHeight} exceeds bundle size`);
            return null;
        }
    ...
    async checkBatchSupport() {
        ...
        try {
            ...
            if (response.ok) {
                if (this.DEBUG) console.log('[CSPScanner] ✅ Batch CSP endpoint available - using batch mode');
                return true;
            } else {
                return false;
            }
        ...
        return new Promise((resolve, reject) => {
            ...
            const initHandler = (e) => {
                ...
                if (msg.type === 'NEED_WASM') {
                    ...
                    if (this.DEBUG) console.log(`[CSPScanner] Worker ${id} ready (WASM: ${msg.version})`);
                    ...
                    if (!msg.hasCarrotKey && this.DEBUG) {
                        console.warn(`[CSPScanner] ⚠️ Worker ${id} has NO Carrot key - Carrot transactions will be missed!`);
                    }
                    if (msg.hasStakeFilter && this.DEBUG) {
                        console.log(`[CSPScanner] Worker ${id} has stake filter enabled`);
                    }
                    if (msg.hasOwnershipCheck && this.DEBUG) {
                        console.log(`[CSPScanner] ✅ Worker ${id} has ownership verification enabled (${msg.subaddressCount} subaddresses)`);
                    }
                    ...
                    worker.addEventListener('error', (err) => {
                        ...
                        console.error('[CSPScanner] ❌ Worker crashed during scan:', details);
    ...
    async scanKeyImagesOnly(startHeight, endHeight, keyImagesCsv) {
        ...
        if (this.DEBUG) console.log(`[CSPScanner] 🔍 Phase 1b FAST: Key-image scan from ${startHeight} to ${endHeight} (${totalChunks} chunks)`);
            ...
            const handleMessage = (e) => {
                ...
                if (msg.type === 'KEY_IMAGES_RESULT') {
                    ...
                    if (msg.spent && msg.spent.length > 0) {
                        ...
                        if (this.DEBUG) console.log(`[CSPScanner] 💸 Found ${msg.spent.length} spent outputs at batch ${msg.startHeight}`);
                    }
                    ...
                    if (completedBatches >= batches.length && pendingBatches === 0) {
                        ...
                        if (this.DEBUG) console.log(`[CSPScanner] ✅ Phase 1b FAST complete: ${allSpent.length} spent found, ${totalInputsScanned} inputs scanned in ${(totalMs / 1000).toFixed(1)}s`);
                        resolve({
                            spent: allSpent,
                            stats: {
                    ...
                    pendingBatches--;
                    console.warn(`[CSPScanner] Key image scan error:`, msg.error);
    ...
    handleScanBatchResult(msg) {
        ...
        if (this.DEBUG && (matches?.length > 0 || this.stats.completedChunks % 100 < chunksProcessed)) {
            console.log(`[CSPScanner] 📦 Batch ${startHeight}-${endHeight}: chunks=${chunksProcessed}, blocks=${blocksProcessed}, matches=${matches?.length || 0}`);
        }
        ...
        if (workerState) {
            ...
            if (currentTask && currentTask.isBatch && chunksProcessed > 0 && chunksProcessed < currentTask.chunkCount) {
                ...
                if (this.DEBUG) console.warn(`[CSPScanner] Worker ${workerId} partial batch: ${chunksProcessed}/${currentTask.chunkCount} chunks. Re-queuing`);
                ...
                if (this.DEBUG) console.log(`[CSPScanner] Batch at ${startHeight} returned 0 chunks (beyond chain tip or error) - not re-queuing`);
            }
    ...
    handleScanError(msg) {
        ...
        console.error(`[CSPScanner] Worker ${workerId} error at height ${startHeight} (retry ${currentRetries}/${MAX_RETRIES}):`, error);
        ...
        if (isRetryable && currentRetries < MAX_RETRIES && failedTask) {
            ...
            console.warn(`[CSPScanner] 🔄 Scheduling retry ${currentRetries + 1}/${MAX_RETRIES} for batch ${startHeight} in ${delay}ms`);
            ...
            if (currentRetries >= MAX_RETRIES) {
                console.error(`[CSPScanner] ❌ SKIPPING batch ${startHeight} after ${MAX_RETRIES} failed retries`);
            }
    ...
    scheduleNextTask() {
        ...
        if (this.DEBUG && (this.stats.completedChunks % 100 === 0)) {
            ...
            console.log(`[CSPScanner] 🚀 Worker ${freeWorker.id} starting ${mode} at ${task.startHeight}`);
        }
        ...
        if (task.bundleData) {
            ...
            if (cspData) {
                ...
                if (this.DEBUG) console.warn(`[CSPScanner] Bundle chunk ${task.startHeight} not found - falling back to fetch`);
            ...
            this._watchdogInterval = setInterval(() => {
                ...
                for (const w of this.workers) {
                    if (w.busy && w.taskStartTime && (now - w.taskStartTime) > 120000) {
                        console.error(`[CSPScanner] ⚠️ WATCHDOG: Worker ${w.id} stuck for >120s on ${w.currentTask?.startHeight}`);
                        ...
                        this.pendingTasks--;
                        if (stuckTask) {
                            if (this.DEBUG) console.log(`[CSPScanner] Re-queuing stuck task: ${stuckTask.startHeight}`);
    ...
    async scan(startHeight, endHeight) {
        ...
        if (useStreaming) {
            if (this.DEBUG) console.log(`[CSPScanner] 🚀 STREAMING MODE: Starting bundle stream (${chunksNeeded} chunks needed)...`);
            ...
            if (bundle && bundle.chunkCount > 0) {
                if (this.DEBUG) console.log(`[CSPScanner] 🚀 STREAMING MODE: ${bundle.chunkCount} chunks processed via stream`);
                ...
                if (bundleEndHeight < endHeight) {
                    ...
                    if (this.DEBUG) console.log(`[CSPScanner] 📦 Bundle ends at ${bundle.lastHeight}, need ${gapChunks} more chunks to reach ${endHeight}`);
                    ...
                    if (this.DEBUG) console.log(`[CSPScanner] 📦 Queued ${this.taskQueue.length} batch tasks for post-bundle gap`);
                }
            } else {
                if (this.DEBUG) console.log(`[CSPScanner] 📦 Streaming failed, falling back to batch mode`);
            }
        }
        ...
        if (this.taskQueue.length === 0 && this.stats.completedChunks === 0) {
            if (this.DEBUG) console.log(`[CSPScanner] 📦 Using ${chunksNeeded <= 10 ? 'incremental' : 'batch'} mode for ${chunksNeeded} chunks`);
            if (this.useBatchMode) {
                ...
                if (this.DEBUG) console.log(`[CSPScanner] BATCH MODE: ${this.totalBlocks.toLocaleString()} blocks in ${this.taskQueue.length} batch requests (${this.batchSize} chunks/batch)`);
            } else {
                // SINGLE CHUNK MODE: Legacy behavior
                ...
                if (this.DEBUG) console.log(`[CSPScanner] Starting scan: ${this.totalBlocks.toLocaleString()} blocks in ${this.stats.totalChunks} chunks`);
            }
        }

        if (this.DEBUG) console.log(`[CSPScanner] Using ${this.workerCount} workers, ${this.chunkSize} blocks/chunk`);
    ...
    finishScan() {
        ...
        if (this.DEBUG) console.log(`[CSPScanner] ✅ Phase 1: ${this.scannedBlocks.toLocaleString()} blocks in ${elapsedSec.toFixed(1)}s (${blocksPerSec.toFixed(0)} blk/s) | ${this.stats.viewTagMatches} matches → ${this.matchedChunks.size} chunks`);

        // Report any failed batches that couldn't be retried
        if (this.failedBatches && this.failedBatches.length > 0) {
            console.warn(`[CSPScanner] ⚠️ ${this.failedBatches.length} batches failed after max retries:`);
            for (const fb of this.failedBatches) {
                console.warn(`  - Batch ${fb.startHeight}: ${fb.error} (${fb.retries} retries)`);
            }
        }

        if (this.DEBUG) {
            console.log(`[CSPScanner] Details: ${this.stats.totalTxs.toLocaleString()} txs, ${this.stats.totalOutputs.toLocaleString()} outputs, ${this.stats.derivations.toLocaleString()} derivations`);
            console.log(`[CSPScanner] Network: ${(this.stats.bytesReceived / 1024 / 1024).toFixed(2)} MB, fetch ${(this.stats.fetchTimeMs / 1000).toFixed(1)}s, WASM ${(this.stats.scanTimeMs / 1000).toFixed(1)}s`);
            ...
            if (this.stats.carrotCoinbaseChecked > 0 || this.stats.carrotRingctPassthrough > 0) {
                ...
                console.log(`[CSPScanner] Carrot: ${this.stats.carrotCoinbaseChecked} coinbase checked (${this.stats.carrotCoinbaseMatched} matched, ${carrotFiltered} filtered), ${this.stats.carrotRingctPassthrough} RingCT passthrough`);
            }
        }
    ...
    abort() {
        ...
        if (this.DEBUG) console.log('[CSPScanner] Aborting scan...');
        this.scanAborted = true;
        this.taskQueue = [];
