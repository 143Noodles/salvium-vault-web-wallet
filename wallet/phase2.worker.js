...
async function loadWasm() {
  ...
  try {
    // Import the WASM factory
    importScripts('/vault/wallet/SalviumWallet.js?v=3.5.15-parallel-phase2');

    // Initialize WASM
    wasmModule = await SalviumWallet({
      locateFile: (path) => {
        if (path.endsWith('.wasm')) {
          return `/vault/wallet/${path}?v=3.5.15-parallel-phase2`;
        }
        return path;
      }
    });

    console.log('[Phase2 Worker] WASM loaded:', wasmModule.get_version ? wasmModule.get_version() : 'unknown');
    return true;
  } catch (e) {
    console.error('[Phase2 Worker] Failed to load WASM:', e);
    return false;
  }
}
...
async function initWallet(seedHex, password) {
  ...
  try {
    ...
    isInitialized = true;
    console.log('[Phase2 Worker] Wallet initialized:', parsed.address?.substring(0, 10) + '...');
    return true;
  } catch (e) {
    console.error('[Phase2 Worker] Failed to init wallet:', e);
    throw e;
  }
}

// Process sparse transaction data
function processSparseData(sparseData, startHeight) {
...
self.onmessage = async function (e) {
  ...
  try {
    switch (type) {
      case 'init': {
        // Load WASM and initialize wallet
        ...
        self.postMessage({ type: 'init_result', id, success: true });
        break;
      }
      ...
      case 'process': {
        ...
        const result = processSparseData(
...
console.log('[Phase2 Worker] Worker started');
