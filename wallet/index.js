        ...
        if (isNode) {
            ...
            SalviumClient({
                ...
                print: (text) => {
                    ...
                    console.log('[WASM]', cleanText);
                },
                printErr: (text) => {
                    // Remove trailing newlines that printf adds
                    const cleanText = text.replace(/\n$/, '');
                    console.error('[WASM ERROR]', cleanText);
                },
                locateFile: (path) => {
...
const salvium_core_js =
{
    ...
    load_return_output_map: function (address) {
        ...
        try {
            ...
            console.warn('⚠️ Failed to load return_output_map from localStorage:', e);
        }
    },
    ...
    save_return_output_map: function (address) {
        ...
        try {
            ...
            console.warn('⚠️ Failed to save return_output_map to localStorage:', e);
        }
    },
    ...
    verify_output_with_base: function (output_key_hex, shared_secret_hex, input_context_hex, base_pubkey_hex) {
        return salvium_utils_promise.then(function (coreBridge) {
            if (typeof coreBridge.verify_output_with_base !== 'function') {
                console.warn('⚠️ verify_output_with_base not available in WASM module');
                return 'false';
            }
            try {
                return coreBridge.verify_output_with_base(output_key_hex, shared_secret_hex, input_context_hex, base_pubkey_hex);
            } catch (e) {
                console.warn('⚠️ verify_output_with_base threw:', e.message);
                return 'false';
            }
        });
    },
    ...
    address_and_keys_from_seed: function (seed, nettype) {
        return salvium_utils_promise.then(function (coreBridge) {
            try {
                ...
                console.error('address_and_keys_from_seed error:', e);
                throw e;
            }
        });
    },
    ...
    seed_and_keys_from_mnemonic: function (mnemonic, wordset_name) {
        return salvium_utils_promise.then(async function (coreBridge) {
            try {
                ...
                if (typeof result === 'string') {
                    ...
                    if (result.startsWith('{') || result.startsWith('[')) {
                        try {
                            ...
                            console.error('Failed to parse mnemonic JSON result:', parseError);
                            throw parseError;
                        }
                    }
                }
                ...
                console.error('seed_and_keys_from_mnemonic error:', e);
                throw e;
            }
        });
    },
    ...
    mnemonic_from_seed: function (seed, wordset_name) {
        return salvium_utils_promise.then(function (coreBridge) {
            try {
                ...
                console.error('mnemonic_from_seed error:', e);
                throw e;
            }
        });
    },
    ...
    newly_created_wallet: function (locale_language_code, nettype) {
        return salvium_utils_promise.then(function (coreBridge) {
            try {
                ...
                console.error('newly_created_wallet error:', e);
                throw e;
            }
        });
    },
    ...
    get_wallet_daemon_urls: function () {
        console.warn('get_wallet_daemon_urls() is deprecated. WASM handles all wallet operations directly.');
                ...
                try {
                    ...
                    try {
                        ...
                        if (useWasm && Module && Module.serialize_get_hashes_fast_binary && request_data.type === 'get_hashes_fast') {
                            ...
                            if (sigA !== '01110101' || sigB !== '01010201' || version !== '01') {
                                ...
                                console.warn('⚠️ WASM signature incorrect, falling back to JavaScript serialization');
                                ...
                                if (convertedSigB !== '01010201') {
                                    console.warn('⚠️ Byte conversion issue detected! Falling back to JavaScript');
                                    binaryRequest = this.serialize_epee_request(request_data);
                                } else {
                            ...
                            try {
                                ...
                                if (sigA !== '01110101' || sigB !== '01010201' || version !== '01') {
                                    console.warn('⚠️ WASM signature incorrect for get_blocks, falling back to JavaScript');
                                    binaryRequest = this.serialize_epee_request(request_data);
                                } else {
                                    const bytes = new Uint8Array(hexString.length / 2);
                                    for (let i = 0; i < hexString.length; i += 2) {
                                ...
                                console.warn('⚠️ WASM serialization failed, using JavaScript fallback:', e.message);
                                binaryRequest = this.serialize_epee_request(request_data);
                            }
                        } else {
                            // Fallback to JavaScript implementation
                            binaryRequest = this.serialize_epee_request(request_data);
                            console.warn('⚠️ Using JavaScript epee serialization (WASM not available)');
                        }
                    } catch (e) {
                    ...
                    if (binaryRequest instanceof ArrayBuffer) {
                        ...
                        if (sigBFromBuffer !== '01010201') {
                            console.warn('⚠️ ArrayBuffer has wrong signature! Falling back to JavaScript');
                            binaryRequest = this.serialize_epee_request(request_data);
                        }
                    }
                    ...
                    if (!response.ok) {
                        ...
                        try {
                            ...
                            console.error(`❌ [Binary RPC Error] Status: ${response.status}, Response body:`, errorText);
                            if (errorText) {
                                try {
                            ...
                            console.warn('Failed to read error response body:', e.message);
                        }
                    ...
                    if (uint8Array.length === 0) {
                        console.error(`❌ [Binary RPC] Empty response from daemon (${response.status})`);
                    ...
                    if (uint8Array.length > 0 && (uint8Array[0] === 0x7b || uint8Array[0] === 0x5b)) { // '{' or '[' character
                        try {
                            ...
                            if (json.error || json.status || json.message) {
                                console.error(`❌ [Binary RPC] Daemon returned JSON error:`, json);
                    ...
                    if (uint8Array.length < 9) {
                        ...
                        try {
                            ...
                            if (text.trim().length > 0 && (text.includes('error') || text.includes('Error') || text.includes('failed'))) {
                                console.error(`❌ [Binary RPC] Daemon returned text error: ${text}`);
                        ...
                        // Log the short response for debugging
                        const preview = Array.from(uint8Array.slice(0, Math.min(64, uint8Array.length)))
                            .map(b => b.toString(16).padStart(2, '0'))
                            .join(' ');
                        console.error(`❌ [Binary RPC] Response too short: ${uint8Array.length} bytes. Hex preview: ${preview}`);
                        console.error(`❌ [Binary RPC] Response as text: ${new TextDecoder('utf8', { fatal: false }).decode(uint8Array)}`);
    ...
    pull_blocks_chunked: async function (address, start_height, target_height, onProgress) {
        ...
        while (current < target_height) {
            ...
            if (current <= start_height) {
                console.warn(`⚠️ [pull_blocks_chunked] Scan not advancing from ${start_height}, forcing +1`);
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (blockIds.length === 0 && currentHeight === 0) {
                    console.log(`📋 [Fast Refresh] No known blocks - skipping fast refresh, will pull blocks directly`);
                    resolve({ success: true, height: currentHeight, skipped: true });
                    return;
                }

                console.log(`📋 [Fast Refresh] Using ${blockIds.length} known block hashes, starting from height ${currentHeight}`);
                ...
                while (currentHeight < stop_height) {
                    ...
                    // Reduced logging - only log on errors or milestones
                    if (hashes.error) {
                        console.error(`❌ [Fast Refresh] Error: ${hashes.error}`);
                    }

                    if (hashes.error) {
                        console.error('❌ Error parsing gethashes.bin response:', hashes.error);
                        break;
                    }
                    ...
                    if (hashes.hashes && hashes.hashes.length > 0) {
                        ...
                        // Only log milestones (every 10000 blocks or at completion)
                        if (currentHeight % 10000 === 0 || currentHeight >= stop_height) {
                            const progressPercent = ((currentHeight - offset) / (stop_height - offset)) * 100;
                            console.log(`📈 Fast refresh: ${currentHeight}/${stop_height} (${progressPercent.toFixed(1)}%)`);
                        }
                    } else {
                        break;
                    }
    ...
    parse_get_hashes_response: function (responseHex) {
        try {
            ...
            if (!responseHex || responseHex.length === 0) {
                console.error(`❌ [Parse Response] Empty or null responseHex. Type: ${typeof responseHex}, Length: ${responseHex ? responseHex.length : 'N/A'}`);
            ...
            if (bytes.length < 9) {
                const hexPreview = Array.from(bytes).map(b => b.toString(16).padStart(2, '0')).join(' ');
                console.error(`❌ [Parse Response] Response too short (${bytes.length} bytes). Hex: ${hexPreview}`);
            ...
            for (let i = 0; i < fieldCount; i++) {
                ...
                if (fieldName === 'm_block_ids' && fieldType === 0x0b) {
                    ...
                    // Unknown type, try to skip (this is a fallback)
                    console.warn(`⚠️ Unknown field type ${fieldType} for field ${fieldName}, skipping`);
                }
            }
            ...
            console.error('❌ Error parsing gethashes.bin response:', error);
            return {
                hashes: [],
                start_height: 0,
                current_height: 0,
    ...
    pull_blocks: async function (address, start_height, onProgress) {
        return new Promise(async (resolve, reject) => {
            try {
                    ...
                    try {
                        ...
                        if (genesisResponse && genesisResponse.block_header && genesisResponse.block_header.hash) {
                            ...
                            if (genesisHash.length === 64 && /^[0-9a-fA-F]{64}$/.test(genesisHash)) {
                                ...
                                console.log(`✅ [Pull Blocks] Cached genesis hash`);
                            }
                        }
                    } catch (e) {
                        console.warn(`⚠️ [Pull Blocks] Could not fetch genesis hash: ${e.message}`);
                    }
                }
                ...
                console.log(`📤 [pull_blocks] Requesting start_height=${start_height}`);
                ...
                console.error('❌ Pull blocks error:', error);
                reject(error);
            }
        });
    },
                ...
                try {
                    ...
                    for (const rpcUrl of urlsToTry) {
                        ...
                        try {
                            ...
                            if (data.error) {
                                console.error('🔗 RPC error in response:', data.error);
                            ...
                            console.warn(`Failed to connect to ${rpcUrl}:`, innerError.message);
    ...
    daemon_rpc_call_wallet: function (method, params, walletUrls) {
        console.warn('daemon_rpc_call_wallet() is deprecated. Use salvium_core_js.get_balance() for balance checking.');
    ...
    scan_block_for_wallet_outputs: async function (block_blob, address, view_key, view_balance_key, block_height = 0, tx_blobs = [], spend_public_key = null) {
        ...
        return salvium_utils_promise.then(async (coreBridge) => {
                ...
                txBlobsArray = tx_blobs
                    .map((tx, index) => {
                        ...
                        if (blobHex.length % 2 !== 0) {
                            console.warn(`⚠️ Skipping tx blob ${index}: odd length ${blobHex.length}`);
                            return null;
                        }

                        if (!/^[0-9a-fA-F]+$/.test(blobHex)) {
                            console.warn(`⚠️ Skipping tx blob ${index}: contains non-hex characters`);
                            return null;
                        }
                        ...
                        if (blobHex.length > MAX_TX_BLOB_LENGTH) {
                            console.warn(`⚠️ Skipping tx blob ${index}: length ${blobHex.length} exceeds limit ${MAX_TX_BLOB_LENGTH}`);
                            return null;
                        }
            ...
            if (Array.isArray(tx_blobs) && tx_blobs.length > 0) {
                const validCount = txBlobsArray.length;
                const invalidCount = tx_blobs.length - validCount;
                if (invalidCount > 0) {
                    console.warn(`⚠️ [WASM] Filtered ${invalidCount} invalid tx blob(s) from ${tx_blobs.length} total (${validCount} valid)`);
                }
            }
            ...
            if (txBlobsArray.length > 0 && typeof coreBridge.scan_transaction === 'function') {
                ...
                try {
                    ...
                    if (!spendPublicKey) {
                        ...
                        if (typeof decoded === 'string') {
                            try {
                                ...
                                console.warn('⚠️ Failed to parse decode_address result:', e);
                                decoded = null;
                            }
                        }
                    ...
                    if (!spendPublicKey || spendPublicKey === 'Not extracted by WASM') {
                            ...
                            try {
                                ...
                                if (minerTxBlobHex && minerTxBlobHex.length > 0) {
                                    try {
                                        ...
                                        console.warn(`⚠️ Error scanning miner tx at height ${block_height}:`, scanErr.message);
                                        // Continue with remaining transactions
                                    }
                                } else {
                                }
                            } catch (minerError) {
                                console.warn(`⚠️ Error extracting miner tx at height ${block_height}:`, minerError.message);
                                // Continue with remaining transactions
                            }
                        } else {
                        }
                            ...
                            try {
                                ...
                                if (protocolTxBlobHex && protocolTxBlobHex.length > 0) {
                                    try {
                                        ...
                                        console.warn(`⚠️ Error scanning protocol tx at height ${block_height}:`, scanErr.message);
                                        // Continue with remaining transactions
                                    }
                                } else {
                                }
                            } catch (protocolError) {
                                console.warn(`⚠️ Error extracting protocol tx at height ${block_height}:`, protocolError.message);
                                // Continue with remaining transactions
                            }
                        } else {
                        }
                        ...
                        // TODO: Extract miner_tx blob from block_blob and scan with new scanner
                        // const txBlobsJson = JSON.stringify(txBlobsArray);
                        // const blockResult = coreBridge.scan_block_for_wallet_outputs(block_blob, address, normalizedViewKey, block_height, txBlobsJson);
    ...
    get_balance: function (address, view_key, view_balance_key, min_height = 0, onProgress, spend_public_key = null) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                console.log(`[SPARC] 📂 Loaded return_output_map: ${this._return_output_map.size} K_r entries`);
                ...
                const isCarrotAddress = address && address.startsWith('SC');
                const correctViewKey = view_key; // For now, assume view_key is already the correct one (set by caller)
                // TODO: In the future, this function should take both view_key and view_balance_key parameters
                ...
                const processScanResult = (batch, actualStartHeight) => {
                    if (!batch.success) {
                        console.warn(`⚠️ Batch scan failed at ${actualStartHeight}: ${batch.error}`);
                ...
                if (useParallelScanner && parallelScanner) {
                    console.log(`🚀 [Parallel Scanner] Starting parallel scan from ${nextStartHeight} to ${currentHeight}`);
                        ...
                        try {
                            ...
                            if (useBatchScanner && blocksResult._raw_response_hex) {
                                ...
                                try {
                                    ...
                                    console.warn(`⚠️ Batch scan exception at ${actualStartHeight}: ${e.message}`);
                            ...
                            if (nextStartHeight <= 363744 && (nextStartHeight + 100) >= 363744) {
                                console.warn(`⚠️ Error pulling blocks from ${nextStartHeight}:`, error.message);
                            }
                ...
                if (pendingProtocolOutputs.length > 0 && this._return_output_map.size > 0) {
                    console.log(`[SPARC] 🔍 Checking ${pendingProtocolOutputs.length} pending PROTOCOL outputs against ${this._return_output_map.size} K_r entries...`);

                    let sparcMatchCount = 0;
                    for (const pOut of pendingProtocolOutputs) {
                        ...
                        if (outputKey && this._return_output_map.has(outputKey)) {
                            ...
                            if (!seenOutputKeys.has(outputKey)) {
                                ...
                                console.log(`[SPARC] ✅ RETURN MATCH @ block ${pOut.height}: Ko=${outputKey.substring(0, 16)}... amount=${(pOut.amount || 0) / 1e8} SAL`);
                            }
                        }
                    }

                    if (sparcMatchCount > 0) {
                        console.log(`[SPARC] 🎉 Found ${sparcMatchCount} stake returns via post-scan lookup!`);
                    } else {
                        console.log(`[SPARC] ℹ️ No SPARC matches in pending PROTOCOL outputs`);
                    }
                }

                // Log final timing summary
                console.log(`⏱️ [Perf] SCAN COMPLETE: ${iterationCount} iterations, Total RPC=${totalRpcTime.toFixed(0)}ms, Total WASM=${totalWasmTime.toFixed(0)}ms`);
                console.log(`⏱️ [Perf] Avg per batch: RPC=${(totalRpcTime / iterationCount).toFixed(0)}ms, WASM=${(totalWasmTime / iterationCount).toFixed(0)}ms`);
                ...
                // Only log if we scanned block 363744
                const scanned363744 = scannedOutputs.some(output => output.height === 363744);
                if (scanned363744) {
                    console.log(`✅ get_balance: Scanned ${scannedOutputs.length} outputs, total balance: ${totalBalance / 1000000000} SAL`);
                }
                resolve(balance);
            } catch (error) {
    ...
    scan_transaction_outputs: async function (address, view_key, tx_data, block_height) {
        ...
        try {
            ...
            console.warn('⚠️ Error scanning transaction outputs:', error.message);
        }
    ...
    extract_tx_public_key: function (tx_data) {
        try {
            ...
            console.warn('Error extracting tx public key:', error.message);
            return null;
        }
    },
    ...
    check_output_ownership: async function (address, view_key, tx_pub_key, output_key, output_index) {
        try {
            ...
            if (txPubKeyBytes.length !== 32 || viewKeyBytes.length !== 32 || outputKeyBytes.length !== 32) {
                console.log('❌ Invalid key lengths');
                return false;
            }
            ...
            console.warn('Error checking output ownership:', error.message);
            return false;
        }
    },
    ...
    scan_block_cryptographically: async function (block_json, address, view_key) {
        try {
            ...
            if (!blockData.result || !blockData.result.block) {
                console.warn('⚠️ Invalid block data format');
                return outputs;
            }
            ...
            console.warn('Error scanning block cryptographically:', error.message);
            return [];
        }
    },
    ...
    scan_transaction_cryptographically: async function (tx_data, address, view_key, block_height, tx_blob_hex = null) {
        ...
        try {
            ...
            if (!txPubKey) {
                console.log('⚠️ No transaction public key found');
                return outputs;
            }
            ...
            console.warn('Error scanning transaction:', error.message);
        }
    ...
    scan_transaction_with_new_scanner: async function (tx_blob_hex, address, view_key, view_balance_key, block_height, spend_public_key = null) {
        ...
        try {
            ...
            if (!spendPublicKey || spendPublicKey === 'Not extracted by WASM') {
                console.warn('⚠️ Could not get spend public key. Please provide spend_public_key parameter.');
                return outputs;
            }

            // Validate inputs
            if (!tx_blob_hex || typeof tx_blob_hex !== 'string') {
                console.warn('⚠️ Invalid transaction blob');
                return outputs;
            }

            if (tx_blob_hex.length % 2 !== 0) {
                console.warn('⚠️ Transaction blob has odd length');
                return outputs;
            }

            if (!view_key || view_key.length !== 64) {
                console.warn('⚠️ Invalid view key length');
                return outputs;
            }

            if (!view_balance_key || view_balance_key.length !== 64) {
                console.warn('⚠️ Invalid view balance key length');
                return outputs;
            }

            if (!spendPublicKey || spendPublicKey.length !== 64) {
                console.warn('⚠️ Invalid spend public key length');
                return outputs;
            }

            // Call new WASM scanner
            if (typeof coreBridge.scan_transaction !== 'function') {
                console.warn('⚠️ scan_transaction function not available in WASM');
                return outputs;
            }
            ...
            if (!resultJson || typeof resultJson !== 'string') {
                console.warn('⚠️ Invalid result from scan_transaction');
                return outputs;
            }
            ...
            console.warn('⚠️ Error in new scanner:', error.message);
            return outputs;
        }
    },
    ...
    get_transfers: function (address, view_key, view_balance_key, min_height = 0, spend_public_key = null) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                console.log(`📊 Starting transfers scan from block height ${min_height} to ${currentHeight}`);
                    ...
                    try {
                        ...
                        if (blocksResult._raw_response_hex) {
                            ...
                            try {
                                ...
                                console.warn(`⚠️ [get_transfers] Batch scan error: ${e.message}`);
                        ...
                        console.warn(`⚠️ [get_transfers] Error at height ${nextStartHeight}: ${error.message}`);
                        nextStartHeight = Math.min(nextStartHeight + 1000, currentHeight);
                    }
                }

                console.log(`✅ get_transfers: Found ${transfers.in.length} incoming transfers (scanned from block ${min_height} to ${currentHeight})`);
                resolve(transfers);
            } catch (error) {
                console.error('❌ get_transfers error:', error);
                reject(error);
            }
        });
    },
    ...
    create_transaction: function (from_address, to_address, amount, nettype) {
        return salvium_utils_promise.then(function (coreBridge) {
            console.log('💰 Creating transaction:', { from: from_address.substring(0, 16) + '...', to: to_address.substring(0, 16) + '...', amount: amount });
    ...
    send_raw_transaction: function (signed_tx_hex) {
        return new Promise(async (resolve, reject) => {
            try {
                console.log('📤 send_raw_transaction: Getting RPC request from WASM');
                ...
                console.log('📤 send_raw_transaction: Sending request to daemon...');

                // Step 2: Send request to daemon
                const response = await this.daemon_rpc_call(rpcRequest.method, rpcRequest.params);

                console.log('📤 send_raw_transaction: Received response, parsing with WASM...');
                ...
                console.log('✅ send_raw_transaction: Successfully sent and parsed result');
                resolve(result);
            } catch (error) {
                console.error('❌ send_raw_transaction error:', error);
                reject(error);
            }
        });
    },
    ...
    get_outputs: function (address, view_key, view_balance_key = null, min_height = 0, spend_public_key = null) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                console.log('🔒 get_outputs: Using privacy-preserving WASM approach (view key stays in browser)');
                console.log('🔒 get_outputs: Will fetch blocks from daemon and scan locally - NOT sending get_outputs to daemon');

                // Step 1: Get current block height (use get_block_count, not get_height)
                // This is a valid daemon RPC method - we're NOT sending get_outputs to the daemon
                const heightResponse = await this.daemon_rpc_call('get_block_count', {});
                const currentHeight = heightResponse.count || heightResponse.height || 0;
                console.log(`📊 Current blockchain height: ${currentHeight}, scanning from height: ${min_height}`);
                ...
                console.log(`🔒 Fetching blocks ${startHeight} to ${currentHeight} for local scanning...`);
                ...
                for (let height = startHeight; height <= currentHeight; height++) {
                    try {
                        ...
                        if (height >= currentHeight) {
                            console.log(`⏭️ Skipping block ${height} (at or above current height ${currentHeight})`);
                            continue;
                        }
                        ...
                        if (blockData.blob && typeof blockData.blob === 'string') {
                            ...
                            // No blob available - this shouldn't happen, but log and skip
                            console.warn(`⚠️ Block ${height}: No blob field in response, skipping...`);
                            continue;
                        }

                        // Validate blob is a valid hex string
                        if (!/^[0-9a-fA-F]+$/.test(blockBlob)) {
                            console.warn(`⚠️ Block ${height}: Blob is not a valid hex string, skipping...`);
                            continue;
                        }
                        ...
                        try {
                            ...
                            if (scanResult && scanResult.error) {
                                console.warn(`⚠️ [WASM] Block ${height} scan failed: ${scanResult.error}`);
                            ...
                            // Log detailed error for WASM scanning failures
                            console.warn(`⚠️ Error scanning block ${height} with WASM:`, scanError.message);
                            console.warn(`   Blob length: ${blockBlob.length} chars (hex)`);
                            console.warn(`   Blob preview: ${blockBlob.substring(0, 100)}...`);
                        ...
                        console.warn(`⚠️ Error fetching/processing block ${height}:`, blockError.message);
                        // Continue with next block
                    }
                }

                console.log(`✅ get_outputs: Found ${outputs.length} unspent outputs`);
                resolve(outputs);
            } catch (error) {
                console.error('❌ get_outputs error:', error);
                reject(error);
            }
        });
    },
    ...
    test_daemon_connectivity: async function () {
        console.log('🔍 Testing daemon connectivity...');
        ...
        try {
            ...
            console.log('Network status:', info.status);
            return { success: true, info: info };
        } catch (error) {
            console.error('❌ Daemon connectivity test failed:', error.message);
            console.error('💡 To fix this:');
            console.error('1. Start CORS proxy: cd built && node cors_proxy.js');
            console.error('2. Or set up wallet daemon as described in WALLET_DAEMON_SETUP.md');
